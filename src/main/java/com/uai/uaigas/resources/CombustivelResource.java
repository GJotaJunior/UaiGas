...
import com.uai.uaigas.services.CombustivelService;
...
public class CombustivelResource {
	...
	public ResponseEntity<CombustivelDTO> findById(@PathVariable Long id) {
		CombustivelDTO dto = service.findById(id);
		return ResponseEntity.ok().body(dto);
	}
	
	@PostMapping
	public ResponseEntity<CombustivelDTO> insert(@Valid @RequestBody CombustivelDTO dto) {
		CombustivelDTO newDto = service.insert(dto);
	...
	@PutMapping(value = "/{id}")
	public ResponseEntity<CombustivelDTO> update(@PathVariable Long id, @Valid @RequestBody CombustivelDTO dto) {
